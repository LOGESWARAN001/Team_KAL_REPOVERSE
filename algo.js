...
let contribs = [];
let heightGrid = null;
let fileMetaGrid = null;

// Look through contributions and assign tileType and seenTile to all positions
export function initializeTiles(newContribs, options = {}) {
    contribs = newContribs;
    heightGrid = options.heightGrid || null;
    fileMetaGrid = options.fileMetaGrid || null;
    tileTypes = [];
    seenTiles = [];
...
function getBuildingHeight(x, y) {
    const raw = getValue(x, y);
    if (raw <= 0) return raw;
    if (heightGrid && isInBounds(x, y) && heightGrid[y][x] > 0) {
        return heightGrid[y][x];
    }
    return raw;
}

function getFileMeta(x, y) {
    if (!fileMetaGrid || !isInBounds(x, y)) return null;
    return fileMetaGrid[y][x];
}
...
function findBuilding(x, y, val) {
    ...
    if (fileMetaGrid) {
        ...
        if (types.length === 0) types = BUILDING_TYPES[res.type];
        const idx = (x * y * val) % types.length;
        res.building = types[idx] || types[0];
        return res;
    }
        ...
        for (let x = 0; x < contribs[y].length; x++) {
            ...
            if (getValue(x, y) > 0 && getSeen(x, y) === 0) {
                ...
                tileTypes[y][x] = {
                    ...
                    dir: building.dir,
                    value: getBuildingHeight(x, y),
                    mirror: building.mirror,
                    fileMeta: getFileMeta(x, y),
                };
            }
        }
    }
